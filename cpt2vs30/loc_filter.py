...
def calc_dist_to_closest_cpt(cpt, all_long_lat_df) -> pd.DataFrame:
    """
    ...
    # subset_lonlats = np.array([subset_lons, subset_lats]).T
    #all_lon_lats = np.array([all_lons, all_lats]).T
    ...
    idx, d = geo.closest_location(locations=all_lon_lats[all_bool_mask], lon=lon,
                                  lat=lat)

    closest_dist_df = pd.DataFrame({"cpt_name": [cpt.name],
...
def calc_all_closest_cpt_dist(cpts: list, all_lon_lat_df: pd.DataFrame, n_procs: Optional[int] = 1) -> pd.DataFrame:

    with multiprocessing.Pool(processes=n_procs) as pool:

        closest_cpt_df_list = pool.map(functools.partial(calc_dist_to_closest_cpt, all_long_lat_df=all_lon_lat_df), cpts)
...
def apply_nztm_to_ll(cpt):
    ...
    return pd.DataFrame({"cpt_name": [cpt.name], "lon": [lon], "lat": [lat]})

def calc_all_ll(cpts: list, n_procs:Optional[int] = 1) -> pd.DataFrame:
...
def get_all_dist_to_closest_cpt(cpts: list, n_procs : int = 1, output_dir : Optional[Path] = None, load_from_previous : Optional[Path] = None) -> pd.DataFrame:
...
# def calc_all_dist_to_closest_cpt(cpt, all_ll_df) -> pd.DataFrame:
#
#     (lat, lon) = nztm_to_ll(cpt.nztm_x, cpt.nztm_y)
#
#     # mask out the row corresponding to the current cpt
#     all_bool_mask = all_ll_df["cpt_name"].to_numpy() != np.array([cpt.name])
#
#     idx, d = geo.closest_location(locations=all_lonlats[all_bool_mask], lon=lon,
#                                   lat=lat)
#
#     closest_dist_df = pd.DataFrame({"cpt_name": [cpt.name],
#                                     "distance_to_closest_cpt_km": [d],
...
#                                     "lon": [lon],
#                                     "lat": [lat],
#                                     "closest_cpt_lon": [all_lonlats[all_bool_mask][idx, 0]],
#                                     "closest_cpt_lat": [all_lonlats[all_bool_mask][idx, 1]]})
#     return closest_dist_df
...
# def OLD_dist_to_closest_cpt(cpts: list, output_dir: Optional[Path]= None) -> pd.DataFrame:
#     """
#     Gets the distance between each CPT and its closest neighbour.
#
#     Parameters
